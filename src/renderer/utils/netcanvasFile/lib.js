...
const getTempDir = async (...args) => {
  const tempPath = await app.getTempPath();
  const dirPath = await path.join(tempPath, 'architect', ...args);
  await fileSystem.mkdirp(dirPath);
  return dirPath;
};
...
const readProtocol = async (workingPath) => {
  const protocolJsonPath = await path.join(workingPath, 'protocol.json');

  try {
    return await fileSystem.readJson(protocolJsonPath);
  } catch (error) {
...
const writeProtocol = async (workingPath, protocol) => {
  ...
  try {
    ...
    await fileSystem.writeJson(protocolJsonPath, prunedProtocol, { spaces: 2 });
...
const deployNetcanvas = async (netcanvasExportPath, destinationUserPath) => {
  ...
  const f = await path.parse(destinationUserPath);
  const backupPath = await path.join(f.dir, `${f.name}.backup-${new Date().getTime()}${f.ext}`);

  const exists = await fileSystem.pathExists(destinationUserPath);
  let createdBackup = false;

  if (exists && createBackup) {
    await fileSystem.rename(destinationUserPath, backupPath);
    createdBackup = true;
  }

  await fileSystem.copy(netcanvasExportPath, destinationUserPath);
...
const commitNetcanvas = async ({ savePath, backupPath }) => {
  ...
  const stat = await fileSystem.stat(savePath);
  if (!stat.isFile()) { 
...
const revertNetcanvas = async ({ savePath, backupPath }) => {
  ...
  const stat = await fileSystem.stat(backupPath);
  if (!stat.isFile()) { 
    throw new Error(`"${backupPath}" (backupPath) does not exist`); 
  }
  
  await fileSystem.unlink(savePath);
  await fileSystem.rename(backupPath, savePath);
  return savePath;
};
...
const createNetcanvasExport = async (workingPath, protocol) => {
  if (!protocol) { 
    throw new Error('Protocol is required for export'); 
  }

  await writeProtocol(workingPath, protocol);
  const exportDir = await getTempDir('exports');
  const exportPath = await path.join(exportDir, uuidv4());
...
const importNetcanvas = async (filePath) => {
  try {
    ...
    const destinationPath = await path.join(protocolsDir, uuidv4());
