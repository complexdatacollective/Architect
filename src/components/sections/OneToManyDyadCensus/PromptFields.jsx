...
const PromptFields = ({ form, variableOptions }) => {
  const sortMaxItems =
    getSortOrderOptionGetter(variableOptions)('property').length;
  const getFormValue = formValueSelector(form);
  const edgeVariable = useSelector((state) =>
    getFormValue(state, 'createEdge'),
  );
      ...
      <Section title="One to Many Dyad Census Prompts">
        <Row>
          <p>
            ...
            (for example, &apos;friendship&apos;, &apos;material support&apos;
            or &apos;conflict&apos;). Enter prompt text below, and select an
            edge type that will be created when the participant selects a target
            node.
          </p>
          <Tip type="info">
            <p>
