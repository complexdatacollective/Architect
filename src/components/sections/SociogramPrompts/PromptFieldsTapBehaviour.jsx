...
const createVariableHandler =
  ...
  async (variableName, variableType, field) => {
    ...
    const { variable } = await dispatch(
      codebookActions.createVariable(entity, type, configuration),
    );
...
const TapBehaviour = ({ form, type, entity }) => {
  ...
  const hasCreateEdgeBehaviour = useSelector(
    (state) => !!getFormValue(state, 'edges.create'),
  );
  const hasToggleAttributeBehaviour = useSelector(
    (state) => !!getFormValue(state, 'highlight.allowHighlighting'),
  );
  const highlightVariable = useSelector((state) =>
    getFormValue(state, 'highlight.variable'),
  );

  const highlightVariablesForSubject = useSelector((state) =>
