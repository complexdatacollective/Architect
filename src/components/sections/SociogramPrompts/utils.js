...
const getEdgeFilteringWarning = (filters, edges) => {
  const existFilters = filters.filter(
    (rule) => rule.options.operator === 'EXISTS',
  );
  const doesNotExistFilters = filters.filter(
    (rule) => rule.options.operator === 'NOT_EXISTS',
  );

  // if any edge should show a warning, return true
  return (
    edges?.some((edge) => {
      const isEdgeInExistFilters = existFilters.some(
